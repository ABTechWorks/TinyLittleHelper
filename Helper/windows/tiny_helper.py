...
def enable_windows_autostart():
    try:
        ...
        install_dir = appdata / "TinyLittleHelper"
        install_dir.mkdir(parents=True, exist_ok=True)

        target_path = install_dir / "TinyLittleHelper.exe"

        # If running as exe, copy itself
        if getattr(sys, "frozen", False):
            current_exe = Path(sys.executable)
            if not target_path.exists():
                shutil.copy2(current_exe, target_path)
        else:
            # Running as script (dev mode) – do nothing
            return
